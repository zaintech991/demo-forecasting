...
async def test_weather_api():
    ...
    if weather_api_key:
        ...
        if len(openweather_key) != 32:
            print(
                f"   ⚠️  Warning: OpenWeatherMap key should be 32 characters, got {len(openweather_key)}"
            )
        ...
        print(
            f"   💡 Add WEATHER_API_KEY=your_key_here or OPENWEATHER_API_KEY=your_key_here to .env file"
        )
        return
                ...
                async with session.get(base_url, params=params) as response:
                    if response.status == 200:
                        ...
                        if service_type == "weatherapi":
                            ...
                            print(
                                f"   ✓ Success: {current['temp_f']:.1f}°F, {current['condition']['text']}"
                            )
                        else:
                            print(
                                f"   ✓ Success: {data['main']['temp']:.1f}°F, {data['weather'][0]['description']}"
                            )
                    else:
                        ...
                        if response.status == 401:
                            ...
                            if service_type == "weatherapi":
                                print(
                                    f"   💡 Get a new key at: https://www.weatherapi.com/"
                                )
                            else:
                                print(f"   💡 New keys take 10-60 minutes to activate")
                                print(
                                    f"   💡 Get a new key at: https://openweathermap.org/api"
                                )
    ...
    print(f"\n4. Recommendations:")
    print(
        f"   • WeatherAPI: Get a new key from https://www.weatherapi.com/ (instant activation)"
    )
    print(
        f"   • OpenWeatherMap: Get a new key from https://openweathermap.org/api (10-60 min activation)"
    )
