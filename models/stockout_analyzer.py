...
def estimate_demand_during_stockout(df, product_id=None, store_id=None):
    ...
    if "dt" in stock_df.columns:
        daily_patterns = (
            stock_df[~stock_df["is_stockout"]]
            .groupby(stock_df["dt"].dt.dayofweek)["sale_amount"]
            .median()
        )
    else:
        # If no date column, use overall median
        daily_patterns = pd.Series(
            [stock_df[~stock_df["is_stockout"]]["sale_amount"].median()]
        )
    ...
    for idx, row in stock_df[stock_df["is_stockout"]].iterrows():
        ...
        if pattern_value is not None:
            ...
            adjustment = (
                total_hours / (total_hours - hours_out)
                if hours_out < total_hours
                else 2
            )
    ...
    stock_df["lost_sales"] = stock_df["lost_sales"].clip(
        lower=0
...
def analyze_stockouts(df):
    ...
    avg_lost_sales = (
        df[df["is_stockout"]]["lost_sales"].mean() if stockout_days > 0 else 0
    )
