...
def preprocess_sales_data(df):
    ...
    missing_cols = [
        col
        for col in ["store_id", "product_id", "dt", "sale_amount"]
    ...
    if (
        "stock_hour6_22_cnt" not in sales_data.columns
        and "stock_hour6_22_cnt" in df.columns
    ):
    ...
    sales_data["stock_hour14_22_cnt"] = (
        sales_data["stock_hour14_22_cnt"].clip(0).astype(int)
    )
