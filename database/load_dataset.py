...
def preprocess_data(df):
    ...
    if (
        "sale_date" in processed_df.columns
        and not pd.api.types.is_datetime64_any_dtype(processed_df["sale_date"])
    ):
        ...
        store_hierarchy = pd.DataFrame(
            {
                ...
                "store_type": [
                    ...
                    for _ in range(len(store_ids))
                ],
                "store_name": [
                    f"Store_{store_id}" for store_id in store_ids["store_id"]
                ],
                "region": [
                ...
                "opening_date": [
                    (
                        datetime(2018, 1, 1)
                        + timedelta(days=np.random.randint(0, 365 * 3))
                    ).date()
                    for _ in range(len(store_ids))
                ],
...
def transform_sales_data(df):
    ...
    sales_data["sale_qty"] = np.ceil(
        sales_data["sale_amount"] / np.random.uniform(0.5, 10.0, size=len(sales_data))
    )

    # Add original price
    sales_data["original_price"] = sales_data["sale_amount"] / (
    ...
    sales_data["stock_hour6_14_cnt"] = np.ceil(
        sales_data["stock_hour6_22_cnt"]
        * np.random.uniform(0.3, 0.7, size=len(sales_data))
    )
    sales_data["stock_hour14_22_cnt"] = (
        sales_data["stock_hour6_22_cnt"] - sales_data["stock_hour6_14_cnt"]
    )
    ...
    if "holiday_flag" in df.columns:
        ...
        sales_data["holiday_flag"] = sales_data["sale_date"].apply(
    ...
    if "promo_flag" in df.columns:
        ...
        sales_data["promo_flag"] = np.random.choice(
            [True, False], size=len(df), p=[0.2, 0.8]
        )
            ...
            promotion_data.append(
                {
                    ...
                    "display_location": np.random.choice(
...
def upload_to_supabase(data_dict, batch_size=1000):
    ...
    if "store_hierarchy" in data_dict and not data_dict["store_hierarchy"].empty:
        ...
        data_dict["store_hierarchy"].to_sql(
            "store_hierarchy",
            engine,
            if_exists="append",
            index=False,
    ...
    if "product_hierarchy" in data_dict and not data_dict["product_hierarchy"].empty:
        ...
        data_dict["product_hierarchy"].to_sql(
            "product_hierarchy",
            engine,
            if_exists="append",
            index=False,
    ...
    if "promotion_data" in data_dict and not data_dict["promotion_data"].empty:
        ...
        data_dict["promotion_data"].to_sql(
            "promotion_events",
            engine,
            if_exists="append",
            index=False,
