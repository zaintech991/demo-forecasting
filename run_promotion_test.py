    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if "promotional_analysis" in result:
                print(
                    f"Found {len(result['promotional_analysis'])} promotional analyses"
                )

                for i, analysis in enumerate(result["promotional_analysis"]):
                    print(f"\nAnalysis {i+1}:")
                    print(
                        f"  Product: {analysis.get('product_name', 'Unknown')} (ID: {analysis.get('product_id')})"
                    )
                    print(
                        f"  City: {analysis.get('city_name', 'Unknown')} (ID: {analysis.get('city_id')})"
                    )
                    print(f"  Recommendation: {analysis.get('recommendation', 'N/A')}")
                    print(f"  Revenue Impact: ${analysis.get('revenue_impact', 0):.2f}")
                    print(
                        f"  Sales Increase: {analysis.get('sales_increase_percentage', 0):.1f}%"
                    )
